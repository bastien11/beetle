...
struct CollisionBitMask {
    ...
    static let beerCategory:UInt32 = 0x1 << 2
...
extension GameScene{
    ...
    func createBird() -> SKSpriteNode {
        ...
        bird.size = CGSize(width: 50, height: 60)
        ...
        bird.physicsBody?.contactTestBitMask = CollisionBitMask.pillarCategory | CollisionBitMask.beerCategory | CollisionBitMask.groundCategory
    ...
    func createWalls() -> SKNode  {
        let beerNode = SKSpriteNode(imageNamed: "beer")
        beerNode.size = CGSize(width: 60, height: 60)
        beerNode.position = CGPoint(x: self.frame.width + 25, y: self.frame.height / 2)
        beerNode.physicsBody = SKPhysicsBody(rectangleOf: beerNode.size)
        beerNode.physicsBody?.affectedByGravity = false
        beerNode.physicsBody?.isDynamic = false
        beerNode.physicsBody?.categoryBitMask = CollisionBitMask.beerCategory
        beerNode.physicsBody?.collisionBitMask = 0
        beerNode.physicsBody?.contactTestBitMask = CollisionBitMask.birdCategory
        beerNode.color = SKColor.blue
